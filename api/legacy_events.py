...
import math
from math import degrees, radians, sin, cos, log10
...
class Events:
    ...
    def __init__(self, lat:float, lng:float, elevation:float, reference_ambient:float, reference_pressure:float):
        self.siteLatitude = round(lat, 8)    #  34 20 34.569   #34 + (20 + 34.549/60.)/60.
        self.siteLongitude = round(lng, 8)   #-(119 + (40 + 52.061/60.)/60.) 119 40 52.061 W
        self.siteElevation =  round(elevation, 3)
        self.siteRefTemp =  round(reference_ambient, 2)       #These should be a monthly average data.
        self.siteRefPress =  round(reference_pressure, 2)
    ...
    def _sortTuple(self, tup):
        ''' 
        Function to sort the list by second item of tuple. Used as a helper
        function in sort methods, not by itself.
         '''
    ...
    def _djd_to_unix(self, djd: float) -> float:
        ''' Convert Dublin Julian Days to unix timestamp (seconds)'''
        return (djd - 25567.5) * 86400*1000
    ...
    def _getDayNow(self):
        ...
        # UTC noon (previous), JD
        intDay = int(ephem.now())

        # How long since previous UTC noon
        dayFrac = ephem.now() - intDay

        # If it's been more/less than 5 hours since previous UTC noon:
        if dayFrac < 0.20833:
            return intDay - 0.55
        else:
            return intDay + 0.45
    ...
    def display_events(self):  
        ...
        # eve_skyFlatBegin, morn_skyFlatEnd
        ptr.horizon = '-1.5'
        sun.compute(ptr)
        ...
        # eve_skyFlatEnd, morn_skyFlatBegin
        ptr.horizon = '-11.75'
        sun.compute(ptr)
        ...
        eveFlatEndDec, \
        eveRaDot, \
        eveDecDot = self._calcEveFlatValues(
                        ptr, 
                        sun, 
                        ops_win_begin, 
                        eve_skyFlatEnd, 
        ...
        print('eveFlatStartRa: ', eveFlatStartRa)
        print('eveFlatStartDec: ', eveFlatStartDec)
        print('eveFlatEndRa: ', eveFlatEndRa)
        print('eveFlatEndDec: ', eveFlatEndDec)
        print('eveRaDot: ', eveRaDot)
        print('eveEndDot: ', eveDecDot)
        print('opsWinBegin: ', ops_win_begin)
        print('eveSkyFlatEnd: ', float(eve_skyFlatEnd))
        ...
        mornFlatEndDec, \
        mornRaDot, \
        mornDecDot = self._calcMornFlatValues(
                        ptr, 
                        sun, 
                        morn_skyFlatBegin, 
                        sunrise, 
        ...
        print('\nmornFlatStartRa: ', mornFlatStartRa)
        print('mornFlatStartDec: ', mornFlatStartDec)
        print('mornFlatEndRa: ', mornFlatEndRa)
        print('mornFlatEndDec: ', mornFlatEndDec)
        print('mornRaDot: ', mornRaDot)
        print('mornEndDot: ', mornDecDot)
        print('morn_skyFlatBegin: ', float(morn_skyFlatBegin))
        print('sunrise: ', float(sunrise))
        ...
        print('ops_win_begin', ephem.Date(ops_win_begin))
